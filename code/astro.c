...
Boolean dmsToDouble(double *f, char *dms, Boolean sign_present) {
    ...
    char test = 223;
    if ((*dms != ':') && (*dms != '*') && (*dms != test)) return False;
    else dms++;
...
//~ double jd2last(double JulianDay, double ut1, Boolean updateRTC) {
//~ if (updateRTC) {
//~ // UT to local time
...
double ztr(double a) {
    ...
    double Alt1_ = Alt1 - (TrueRefrac(Alt1, 0.0, 0.0) / 60.0);
    double Alt2_ = Alt2 - (TrueRefrac(Alt2, 0.0, 0.0) / 60.0);

    double x = 15.0 * ((double) ((Alt1 - Alt2) / (Alt1_ - Alt2_)));
    if (x > 15.0) x = 15.0;
    return x;
}
...
//~ idouble angDist(double h, double d, double h1, double d1)f ((az_Azm1<-90.0) && (az_Azm2>90.0)) az_Azm1+=360.0;
